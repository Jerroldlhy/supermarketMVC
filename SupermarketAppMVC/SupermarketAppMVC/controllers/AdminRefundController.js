...
const renderView = (res, name, data = {}) => res.render(name, { ...data, user: res.locals.user });
...
const restockRequestItems = (requestId, callback) => {
    refundRequestItemModel.getByRequestId(requestId, (err, items = []) => {
        ...
        items.forEach((item) => {
            ...
            Product.increaseQuantity(productId, quantity, (updateErr) => {
                if (failed) {
                    return;
                }
                if (updateErr) {
...
const finalizeRefund = (req, res, requestId, adminNote, restockItems, successMessage) => {
    refundRequestModel.updateStatus(requestId, 'COMPLETED', adminNote, () => {
        refundRequestModel.getById(requestId, (requestErr, request) => {
            ...
            refundModel.getByRequestId(requestId, (refundErr, refunds = []) => {
                ...
                refundRequestItemModel.getByRequestId(requestId, (itemsErr, items = []) => {
                    ...
                    const invoiceNumber = `RF-${requestId}`;
                    const refundPayload = {
                        ...latestRefund,
                        invoiceNumber,
                        refundReason: request.reason || adminNote || null,
                        paymentMethod: request.paymentMethod || 'Unknown'
                    };

                    const orderTotal = Number(request.total || 0);
                    const refundedToDate = Number(request.refundedAmount || 0);
                    const remaining = Number(Math.max(0, orderTotal - refundedToDate).toFixed(2));

                    Notifications.sendRefundInvoice(
                        { id: request.userId, username: request.username, email: request.email },
                        refundPayload,
                        items,
                        orderTotal,
                        refundedToDate,
    ...
    async approve(req, res) {
        ...
        refundRequestModel.getById(requestId, async (err, request) => {
            ...
            if (paymentMethod === 'paypal' && captureId) {
                try {
                    const result = await paypalRefund.refundCapture(captureId, amount, 'SGD');
